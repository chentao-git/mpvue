<template>
  <div class="index">
    <dl class="ub-box ub-col">
      <dd class="z-width-100-percent z-padding-all-8-px z-bg-color-fff ub-box ub-ver z-box-sizing-border" style="height: 78rpx;">
        <div class="ub-flex-1 search ub-box ub-ver-v">
          <i class="iconfont icon-sousuo z-color-666 z-font-size-16"></i>
          <input @input="doInput" @confirm="doSearch" class="ub-flex-1 z-font-size-14 z-color-666 z-padding-v-5-px z-margin-left-8-px" placeholder="输入搜索条件  "/>
        </div>
        <span @click="backBeaforWin" class="z-font-size-13 z-margin-left-8-px" style="color:#06c1ae">取消</span>
      </dd>
    </dl>
      <!--搜索值不为空的时候，显示搜索列表-->
      <div v-if="searchVal.length>0" class="ub-box ub-col" style="padding:8px 8px 0 8px">
        <scroll-view scroll-y style="height: calc(100vh - 80px)" scroll-top="0">
          <ul class="ub-box ub-col">
            <li @click="clickSearchItem(val)" v-if="currSearchList.length>0" class="search-item ub-box ub-ver z-box-sizing-border" v-for="(val, i) in currSearchList" :key="i">
            ...
            <li @click="clickSearchItem(searchVal)" v-if="currSearchList.length===0" class="search-item ub-box ub-ver z-box-sizing-border">
              <i class="iconfont icon-sousuo z-color-999 z-font-size-16 z-margin-right-10-px"></i>
      ...
      <div v-if="searchVal.length===0" class="ub-box ub-col">
        ...
        <dd class="z-margin-h-8-px z-width-100-percent z-box-sizing-border z-bg-color-fff z-padding-all-8-px ub-box">
          <ul class="ub-box ub-wrap">
            <li @click="clickSearchItem(val)" v-for="(val, idx) in guess" :key="idx" class="item z-font-size-13 z-color-333">{{val.name}}</li>
          </ul>
        </dd>
        ...
        <dd class="z-margin-h-8-px z-width-100-percent z-box-sizing-border z-bg-color-fff z-padding-all-8-px ub-box">
          <ul class="ub-box ub-wrap">
            <li @click="clickSearchItem(val)" v-for="(val, idx) in history" :key="idx" class="item z-font-size-13 z-color-333">{{val.name}}</li>
          </ul>
        </dd>
      </div>
    </dl>
  ...
  export default {
    ...
    methods: {
      ...
      clickSearchItem (val) {
        ...
        const url = '/pages/index/main?id=' + val.goodId
        // 跳转小程序中的tabBar 页面不能使用redirectTo 重定向 navigateTo 新开一个页面
        this.$commonjs.reLaunchTo(url)
      },
      backBeaforWin () {
        this.$commonjs.backTo() // 返回上一页
      }
    },
    mounted () {
      this.searchVal = ''
